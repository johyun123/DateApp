import { View, Text, Image, Alert, StyleSheet, Pressable, ScrollView } from 'react-native'
...
function SelledPage(){
    ...
    async function deleteTrade(){
        if (!buyerDoc || !docUser || !tradeId) return;

        try{
            const price = (Number)(serviceData.price.replaceAll(",", ""))
            const credit = buyerDoc.credit + price
            const sellerCredit = docUser.sellerCredit - price

            await firestoreDB.updateUserData(buyerDoc.id, { credit })
            await firestoreDB.updateUserData(docUser.id, { sellerCredit })
            await firestoreDB.deleteTrade(tradeId)
        ...
        <ScrollView style={styles.container}>
            ...
            <View style={styles.infoCard}>
                ...
                <View style={styles.infoRow}>
                    ...
                    <Text style={styles.infoLabel}>구매자</Text>
                    <Text style={styles.infoValue}>{tradeData?.buyerName ?? "로딩 중"}</Text>
                </View>
            ...
            {/* Cancel Button */}
            {!isCompleted && (
                <Pressable style={styles.cancelButton} onPress={deleteTrade}>
                    <Ionicons name="close-circle-outline" size={20} color="#FF6B9D" />
...
const styles = StyleSheet.create({
    ...
    cancelButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, marginHorizontal: 24, paddingVertical: 16, borderRadius: 12, borderWidth: 2, borderColor: '#FF6B9D' },
    cancelButtonText: { fontSize: 16, fontWeight: '700', color: '#FF6B9D' },
});
